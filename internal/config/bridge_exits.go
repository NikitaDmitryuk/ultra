...
func buildBridgeExitOutbounds(
	spec *Spec,
	exitNodes []exits.Node,
	activeExitID string,
	w xrayWireResolved,
	buildStream func(pinnedPeerCertSHA256 string) map[string]any,
) []any {
	enabled := exits.FilterEnabled(exitNodes)
	if len(enabled) == 0 {
		return []any{
			bridgeExitVLESSOutbound(
				w.OutboundExitTag,
				spec.Exit.Address,
				spec.Exit.Port,
				spec.Exit.TunnelUUID,
				w,
				buildStream(spec.Exit.PinnedPeerCertSHA256),
			),
		}
	}
	var out []any
	for _, n := range enabled {
		out = append(
			out,
			bridgeExitVLESSOutbound(exits.OutboundTag(n.ID), n.Address, n.Port, n.TunnelUUID, w, buildStream(n.PinnedPeerCertSHA256)),
		)
	}
	...
	if legacyNode.ID != "" {
		out = append(
			...
			bridgeExitVLESSOutbound(
				...
				legacyNode.Address,
				legacyNode.Port,
				legacyNode.TunnelUUID,
				w,
				buildStream(legacyNode.PinnedPeerCertSHA256),
			),
		)
	}
	return out
...
func bridgeExitVLESSOutbound(
	tag, address string,
	port int,
	tunnelUUID string,
	w xrayWireResolved,
	outStream map[string]any,
) map[string]any {
