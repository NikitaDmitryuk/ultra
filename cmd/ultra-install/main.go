...
import (
	...
	"github.com/nikdmitryuk/ultra/internal/install"
	"github.com/nikdmitryuk/ultra/internal/realitykey"
...
func main() {
	...
	vlessPort := flag.Int("vless-port", 443, "TCP port: public listener on bridge and splithttp listener on exit")
	...
	if err := install.RunSSH(*sshUser, *bridgeHost, *identity, "bash", "-lc", bridgePrep); err != nil {
	...
	if err := install.RunSSH(*sshUser, *exitHost, *identity, "bash", "-lc", exitPrep); err != nil {
	...
	if *generateExitTLS {
		...
		if err := install.RunSSH(*sshUser, *exitHost, *identity, "bash", "-lc", ssl); err != nil {
	...
	_ = os.WriteFile(tmpEnv, []byte("ULTRA_RELAY_ADMIN_TOKEN="+adminToken+"\n"), 0o600)
	defer func() {
	...
	if err := install.RunSSH(*sshUser, *bridgeHost, *identity, "bash", "-lc", checkUsers); err != nil {
...
`, *remoteDir)
	...
	if err := install.RunSSH(*sshUser, *bridgeHost, *identity, "bash", "-lc", remoteFinalize); err != nil {
...
chmod 600 "$REMOTE_DIR/spec.json" || true
`, *remoteDir)
	if err := install.RunSSH(*sshUser, *exitHost, *identity, "bash", "-lc", exitFinalize); err != nil {
	...
	for _, h := range []string{*bridgeHost, *exitHost} {
		...
		unitMv := `mv /tmp/ultra-relay.service /etc/systemd/system/ultra-relay.service && systemctl daemon-reload && systemctl enable --now ultra-relay`
		if err := install.RunSSH(*sshUser, h, *identity, "bash", "-lc", unitMv); err != nil {
